...
// DELETE DATA
    ...
    void act_delete (char npm[16]) {
        ...
        sqlite3_prepare_v2(db, sql_c, strlen(sql_c), &fq_c, NULL);
        sqlite3_bind_text(fq_c, 1, npm, strlen(npm), NULL);
        sqlite3_step(fq_c);
        strcpy(nama, sqlite3_column_text(fq_c, 1));
        ...
        if (yakin == 1) {
            ...
            sqlite3_prepare_v2(db, sql_d, strlen(sql_d), &fq_d, NULL);
            sqlite3_bind_text(fq_d, 1, npm, strlen(npm), NULL);
            sqlite3_step(fq_d);
            printf("\n%s, %s\n", sqlite3_expanded_sql(fq_d), npm);
